...
for INDEX in range(0, 701, 700):
    ...
    # design = Design(f'{SCRIPT_DIR}/{INPUT_FILE}')
    ...
    def conservative_cut(color):
        ...
        plot_bar_length(top_rebar, top_length, bot_rebar,
                        bot_length, color=color)
    ...
    def conservative_sol(color):
        ...
        plt.plot(DATASET['StnLoc'] * 100,
                 DATASET['BarTopNum'] * TOP_SIZE, color=color, linewidth=linewidth)
        plt.plot(DATASET['StnLoc'] * 100, -
                 DATASET['BarBotNum'] * BOT_SIZE, color=color, linewidth=linewidth)
    ...
    def compare_linearcut_to_conservative():
        plt.figure()
        zero_line()

        horizontalline()

        plt.axvline((END - START) * 0.1 + START, linestyle='--', color=gray)
        plt.axvline(END - (END - START) * 0.1, linestyle='--', color=gray)
