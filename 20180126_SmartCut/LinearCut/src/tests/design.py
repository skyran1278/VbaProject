...
from rebar import get_diameter, get_area
...
def create_pkl(path):
    ...
    etabs_design = pd.read_excel(excel, sheet_name='etabs_design')

    df = pd.read_excel(excel, sheet_name='多點斷筋', header=[0, 1], usecols=22)
    df = df.rename(columns=lambda x: x if 'Unnamed' not in str(x) else '')

    con = pd.read_excel(excel, sheet_name='傳統斷筋', header=[0, 1], usecols=22)
...
class Design:
    ...
    def __init__(self, path):
        ...
        with open(f'{path}.pkl', 'rb') as f:
            self.etabs_design, self.df, self.conservative = pickle.load(f)

    def funcname(self, index, col=None):
        etabs_design = self.etabs_design.loc[
